...
//   const RNFS = require('react-native-fs');
  const defaultStatus =
    'You can save your current wallet, Import a wallet, Replace your current wallet, or Export a selected wallet';
...
//       console.warn(
//         'SaveScreen - multiple pickers were opened, only the last will be considered'
//       );
//     } else {
//       throw err;
//     }
...
//     selectedItem: ReadDirItem | undefined = selection
//   ): Promise<boolean> {
...
//       if (selectedItem) {
//         console.log(
//           'SaveScreen - setting selection to item',
//           selectedItem.name
//         );
...
//                 updateMessages(
//                   'Successfully exported selected wallet to Downloads'
//                 );
...
//     const dirItems: ReadDirItem[] = await RNFS.readDir(
//       RNFS.DocumentDirectoryPath
//     ); // On Android, use "RNFS.DocumentDirectoryPath" (MainBundlePath is not defined
//     const archiveItems = dirItems.filter((item) => {
//       console.log(
//         'SaveScreen - is an archive item?',
//         item.name,
//         'is file?',
//         item.isFile(),
//         'is match?',
//         item.name.match('^' + RW_BACKUP + '.*.zip') != null
//       );
//       return (
//         item.isFile() && item.name.match('^' + RW_BACKUP + '.*\\.zip') != null
//       );
//     });
//     console.log('SaveScreen - Found archived items:', archiveItems.length);
//     const sortedArchives = archiveItems.sort(
//       (a, b) => -1 * a.name.localeCompare(b.name)
//     );
//     console.log(
//       'SaveScreen - Sorted archived items:',
//       sortedArchives.map((item) => item.name)
//     );
...
//   }
  ...
  function getDescription(item: any) {
    if (item.mtime) {
      return (
        item.mtime.toLocaleDateString('en-us', {
          weekday: 'long',
          year: 'numeric',
          month: 'short',
          day: 'numeric',
...
//         await refreshItems();
//         updateMessages(
//           'Successfully imported wallet, it is listed as the Latest wallet'
//         );
//         return true;
//       }
//     } catch (e) {
...
//       console.log(
//         'SaveScreen - We need to clear files from backup dir',
//         dirItems
//       );
...
//               Alert.alert('Load Successful', 'Restarting RootsWallet', [
//                 {
//                   text: 'Restart',
//                   onPress: () => {
//                     // navigation.dispatch(
...
//       const archivePath = zipPath.replace(
//         ZIP_EXTENSION,
//         Date.now() + ZIP_EXTENSION
//       );
...
//     console.log(
//       'SaveScreen - Pruning archived zips if there are more than',
//       MAX_ARCHIVES,
//       archives
//     );
...
//           console.log(
//             'SaveScreen - Deleted archived backup ',
//             archiveItem.path
//           );
//         } catch (error: any) {
//           console.warn(
//             'SaveScreen - failed to cleanup backup',
//             error,
//             error.stack
//           );
//         }
...
//       if (unzipResult) {
//         const input = await RNFS.readFile(
//           backupPath + '/rootswallet_export.txt',
//           'utf8'
//         );
...
//       console.log(
//         'SaveScreen - Cleaning archived backup dirs in',
//         RNFS.DocumentDirectoryPath
//       );
...
//           const zipFile = await addDataAndZip(
//             RNFS.DocumentDirectoryPath,
//             output
//           );
    ...
    >
      ...
      <Animated.View style={styles.viewAnimated}>
        ...
        <View style={[styles.containerRowSpaced, { marginBottom: 10 }]}>
          ...
          >
            {problem && problem?.length > 0 ? problem : status}
          </Text>
        </View>
        ...
        <View style={[styles.containerRowSpaced, { marginBottom: 10 }]}>
          <FlatList
            ...
            ListEmptyComponent={
              <Text
                style={[styles.itemHighlighted, { alignContent: 'flex-start' }]}
              >
                No Saved Wallets
              </Text>
            }
            renderItem={({ item }) => (
              <View style={styles.containerRow}>
                ...
                //   onPress={() => refreshItems(item)}
                  status={
                    selection && item.name.match(selection.name)
                      ? 'checked'
                      : 'unchecked'
                  }
                />
                <Text
                  style={
                    selection && item.name.match(selection.name)
                      ? [styles.itemHighlighted, styles.black]
                      : [styles.clickableListArchive, styles.black]
                  }
                >
                  {getTitle(item) + '\n' + getDescription(item)}
                </Text>
